...
import type { KChurchAboutContent } from "@/lib/types/cms";

export function KChurchAboutEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<KChurchAboutContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("kChurchAbout").then(setForm);
  }, []);

  if (!form) return <p className="text-neutral-500">Loading…</p>;

  const update = (field: keyof KChurchAboutContent, value: string) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("kChurchAbout", form);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <MediaUpload label="Gallery image 1" type="image" value={form.image1Url} onChange={(url) => update("image1Url", url)} folder="k-church" />
      <MediaUpload label="Gallery image 2" type="image" value={form.image2Url} onChange={(url) => update("image2Url", url)} folder="k-church" />
