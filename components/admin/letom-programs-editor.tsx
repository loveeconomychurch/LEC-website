...
import type { LetomProgramsContent, LetomProgramItem } from "@/lib/types/cms";

const ICON_OPTIONS: LetomProgramItem["icon"][] = ["School", "Users", "Heart"];

export function LetomProgramsEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<LetomProgramsContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("letomPrograms").then(setForm);
  }, []);
  ...
  const updateProgram = (index: number, field: keyof LetomProgramItem, value: string) => {
    setForm((prev) => {
      if (!prev) return prev;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("letomPrograms", form);
        ...
        {form.programs.map((program, i) => (
          <div key={i} className="p-4 rounded-lg border border-neutral-700 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <select
                value={program.icon}
                onChange={(e) => updateProgram(i, "icon", e.target.value as LetomProgramItem["icon"])}
