...
import type { LetomEventsContent, LetomEventItem } from "@/lib/types/cms";

export function LetomEventsEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<LetomEventsContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("letomEvents").then(setForm);
  }, []);

  if (!form) return <p className="text-neutral-500">Loading…</p>;

  const updateSection = (value: string) => {
    setForm((prev) => (prev ? { ...prev, sectionTitle: value } : prev));
  };

  const updateEvent = (index: number, field: keyof LetomEventItem, value: string) => {
    setForm((prev) => {
      if (!prev) return prev;
  ...
  const addEvent = () => {
    setForm((prev) =>
      prev
        ? { ...prev, events: [...prev.events, { date: "", location: "", title: "", imageUrl: "" }] }
        : prev
    );
  };
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("letomEvents", form);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="space-y-2">
        ...
        <Input value={form.sectionTitle} onChange={(e) => updateSection(e.target.value)} className="bg-neutral-800 border-neutral-700" />
      </div>
      <div className="space-y-4">
        ...
        {form.events.map((event, i) => (
          <div key={i} className="p-4 rounded-lg border border-neutral-700 space-y-3">
            ...
            <div className="space-y-2">
              <Label className="text-xs">Date (e.g. March 15, 2024)</Label>
            ...
            <MediaUpload label="Image" type="image" value={event.imageUrl} onChange={(url) => updateEvent(i, "imageUrl", url)} folder="letom" />
          </div>
        ))}
      </div>
