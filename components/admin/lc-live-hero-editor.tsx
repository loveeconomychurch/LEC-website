...
import type { LcLiveHeroContent } from "@/lib/types/cms";

export function LcLiveHeroEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<LcLiveHeroContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("lcLiveHero").then(setForm);
  }, []);

  if (!form) return <p className="text-neutral-500">Loading…</p>;

  const update = (field: keyof LcLiveHeroContent, value: string) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("lcLiveHero", form);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <MediaUpload label="Background image" type="image" value={form.backgroundImageUrl} onChange={(url) => update("backgroundImageUrl", url)} folder="lc-live" />
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <Input value={form.primaryCtaLink ?? ""} onChange={(e) => update("primaryCtaLink", e.target.value)} placeholder="/give" className="bg-neutral-800 border-neutral-700" />
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <Input value={form.secondaryCtaLink ?? ""} onChange={(e) => update("secondaryCtaLink", e.target.value)} placeholder="#how-to-join" className="bg-neutral-800 border-neutral-700" />
      </div>
