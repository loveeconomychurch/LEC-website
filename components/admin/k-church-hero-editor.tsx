...
import type { KChurchHeroContent } from "@/lib/types/cms";

export function KChurchHeroEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<KChurchHeroContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("kChurchHero").then(setForm);
  }, []);

  if (!form) return <p className="text-neutral-500">Loading…</p>;

  const update = (field: keyof KChurchHeroContent, value: string) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("kChurchHero", form);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <MediaUpload label="Background image" type="image" value={form.backgroundImageUrl} onChange={(url) => update("backgroundImageUrl", url)} folder="k-church" />
      <div className="space-y-2">
