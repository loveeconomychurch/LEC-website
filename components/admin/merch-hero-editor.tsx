...
import type { MerchHeroContent } from "@/lib/types/cms";

export function MerchHeroEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<MerchHeroContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("merchHero").then(setForm);
  }, []);

  if (!form) return <p className="text-neutral-500">Loading…</p>;

  const update = (field: keyof MerchHeroContent, value: string) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("merchHero", form);
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="e.g. LEC Merch Store"
