...
import type { LetomAboutContent } from "@/lib/types/cms";

export function LetomAboutEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<LetomAboutContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("letomAbout").then(setForm);
  }, []);

  if (!form) return <p className="text-neutral-500">Loading…</p>;

  const update = (field: keyof LetomAboutContent, value: string) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("letomAbout", form);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <MediaUpload label="Gallery image 1" type="image" value={form.image1Url} onChange={(url) => update("image1Url", url)} folder="letom" />
      <MediaUpload label="Gallery image 2" type="image" value={form.image2Url} onChange={(url) => update("image2Url", url)} folder="letom" />
