...
import type { ChildrensMinistryAgeGroupsContent, ChildrensMinistryAgeGroupItem } from "@/lib/types/cms";

export function ChildrensMinistryAgeGroupsEditor({ onSaved }: { onSaved?: () => void }) {
  const [form, setForm] = useState<ChildrensMinistryAgeGroupsContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  useEffect(() => {
    getContent("childrensMinistryAgeGroups").then(setForm);
  }, []);
  ...
  const updateGroup = (index: number, field: keyof ChildrensMinistryAgeGroupItem, value: string) => {
    setForm((prev) => {
      if (!prev) return prev;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      await setContent("childrensMinistryAgeGroups", form);
